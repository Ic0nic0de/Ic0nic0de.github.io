...
var versionno = String(versionno[0] + "." + versionno[1] + "." + versionno[2]);

	if(versionNumber >= minVersion && version <= maxVersion) {
		document.getElementById('compatibility').innerHTML=VERSION_CHECK_SUPPORTED.replace("%s", versionno);
		document.body.style.setProperty("--title-color", "rgba(39,174,96, 0.5)");
	} else if(versionNumber > maxVersion) {
		document.getElementById('compatibility').innerHTML=VERSION_CHECK_UNCONFIRMED.replace("%s", versionno);
		...
		document.getElementById('compatibility').innerHTML=VERSION_CHECK_UNSUPPORTED.replace("%s", versionno);
		document.body.style.setProperty("--title-color", "rgba(192,57,43, 0.5)");
	}
